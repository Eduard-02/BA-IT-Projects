...
class IntArray():
    ...
    def remove(self, index:int) -> int:
      # 1) Can't remove from an empty array
      if self._size == 0:
        return None
      ...
      # 3) Save the value to return before rebuilding memory
      val = self.__getitem__(index)

      old_resmem = self._resmem
      old_size = self._size
      B = self._bytes_per_element

      # 4) New logical size after removal
      self._size -= 1

      # 5) If size becomes 0, no memory is needed
      if self._size == 0:
        self._resmem = None
        return val

      # 6) Allocate new smaller memory block
      new_resmem = ReservedMemory(self._size * B)

      # 7) Copy left part (bytes before the removed element)
      left_bytes = index * B
      if left_bytes > 0:
        new_resmem.copy(old_resmem, count=left_bytes, source_index=0, destination_index=0)

      # 8) Copy right part (bytes after removed element), shifted left by 1 element
      right_elems = old_size - index - 1
      right_bytes = right_elems * B
      if right_bytes > 0:
        new_resmem.copy(old_resmem, count=right_bytes, source_index=(index + 1) * B, destination_index=index * B)

      # 9) Switch to the new memory block
      self._resmem = new_resmem
